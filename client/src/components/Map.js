  ...
  const getMarkerPoints = () => {
    if (markers) {
      markers.forEach(marker =>
        markerPoints.push({ name: marker[2], position: [marker[0], marker[1]] })
      )
    }
  }
    ...
    <div className='map-container'>
      {!user && (
        <h5 className='login-msg'>
          To view more information, please <a href='/login'>login</a>.
        </h5>
      )}
          ...
          {markers &&
            markers.map((item, index) => (
              <Marker
                ...
                icon={
                  item.name === 'This is your midpoint.'
                    ? starMidIcon
                    : starNearbyIcon
                } // displays a different marker depending on whether the point is the midpoint or nearby location
              >
                <Popup>
